...
class TransportTechnologyRules(GenericRule):
    ...
    def constraint_no_flow_transport(self):
        ...
        def simulate_operation(num_edges, failure_probability, downtime, array, failed_edge=None,
            ...
            - failure_probability: Probability of failure at each time step.
            ...
            if downtime_counter > 0:
                downtime_counter -= 1
                # Append a row with the same failed state as the previous row
            ...
            if np.random.rand() < failure_probability:
                # Choose a random technology to fail
                failed_edge = np.random.randint(num_edges)
                # Set all other technologies to working (1) and the failed one to failed (0)
                row = np.ones((1, num_edges), dtype=int)
                row[0, failed_edge] = 0
                # Append the row to the array
                array = np.vstack([array, row])
                # Start the downtime counter
                downtime_counter = downtime
            else:
                # If no failure, append a row with all technologies working (1)
        ...
        for tech in index.get_unique(["set_transport_technologies"]):
            term_capacity = self.parameters.capacity_limit.loc[tech,: ,edges, times]
            term_flow = self.variables["flow_transport"].loc[tech, edges, times]
            #operation_probability = self.parameters.operation_probability.loc[tech]
            operation_probability = 0.5
            failure_probability = 1-operation_probability
            # TODO downtime
            downtime_transport = 3
            # downtime_transport = self.parameters.downtime_transport

            array = np.ones((1, len(edges)), dtype=int)
            for time_step in times:
                operation = simulate_operation(len(edges), failure_probability, downtime_transport, operation, failed_edge=None, downtime_counter=0)


                ### formulate constraint
                lhs = term_flow.loc[edges,time_step]
                rhs = term_capacity.loc[:,edges,time_step] * operation[time_step]
                constraints.append(lhs <= rhs)
