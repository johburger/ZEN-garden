...
class TechnologyRules(GenericRule):
    ...
    def constraint_opex_yearly_block(self):
        ...
        for tech, year in index.get_unique(["set_technologies", "set_time_steps_yearly"]):
            ...
            if self.system['include_n1_contingency_transport'] or self.system['include_n1_contingency_conversion']:
                term_neg_summed_cost_opex = - ((self.variables["cost_opex"].loc[tech, :, :, times].sum(["set_failure_states"]) -
                ((len(index.get_unique(["set_failure_states"])) - 1) * self.variables["cost_opex"].loc[tech, :, "no_failure_technology: no_failure_location", times]))
                * self.parameters.time_steps_operation_duration.loc[times]).sum(["set_time_steps_operation"])
            else:
