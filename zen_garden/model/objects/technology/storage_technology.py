...
class StorageTechnology(Technology):
    ...
    def construct_vars(cls, optimization_setup):
        ...
        # energy spillage
        variables.add_variable(model, name="flow_storage_spillage", index_sets=(index_values, index_names),
            bounds=(0, np.inf), doc='storage spillage of storage technology on node i in each storage time step', unit_category={"energy_quantity": 1, "time": -1})
        # storage level
        if optimization_setup.system['n1_contingency']:
