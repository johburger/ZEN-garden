...
class TransportTechnologyRules(GenericRule):
    ...
    def constraint_capacity_factor_transport(self):
        ...
        if self.optimization_setup.system['n1_contingency']:
            ...
            term_flow = (1*self.variables["flow_transport"]).broadcast_like(term_capacity.const).where(~mask_flexible_techs)
        ...
        if mask_flexible_techs.any():
            ...
            # the failure modes are also applied to the capacity limit for each edge:
            if self.optimization_setup.system['n1_contingency']:
                term_failure = self.parameters.operation_state.loc[flexible_techs, edges, :].rename({"set_technologies": "set_transport_technologies", "set_location": "set_edges"})
                rhs = self.parameters.capacity_limit.loc[flexible_techs, "power", edges, time_step_year].rename({"set_technologies":"set_transport_technologies","set_location": "set_edges"}) * term_failure
            else:
