...
class DataInput:
    ...
    def extract_general_input_data(
        self, df_input, df_output, file_name, index_name_list, default_value, index_sets
    ):
    ...
    def extract_locations(
        ...
        Args
            ...
            nodes + coordinates \n
            super_locations: boolean to extract super nodes or edges
        """
        if super_locations:
            if extract_nodes:
    ...
    def extract_pwa_capex(self):
        ...
        if df_input_nonlinear is not None:
                ...
                if value_variable == breakpoint_variable:
                    ...
                    if linear_regress_object.slope != 0:
                        relative_intercept = np.abs(
                            linear_regress_object.intercept
                            / linear_regress_object.slope
                        )
                    else:
            ...
            # linear
                ...
                conversion_factor_levels = [
                    linear_dict.index.names[-1]
                ] + linear_dict.index.names[:-1]
            ...
            # no dependent carrier
                ...
                raise NotImplementedError(
                    f"There are both linearly and nonlinearly modeled "
                    f"variables in capex of {self.element.name}. Not yet "
                    "implemented"
                )
        # linear
        else:
    ...
    def read_pwa_capex_files(self):
        ...
        if df_input is not None:
            string_row = (
                df_input.map(lambda x: pd.to_numeric(x, errors="coerce"))
                .isna()
                .any(axis=1)
            )
            if string_row.any():
    ...
    def construct_index_list(self, index_sets):
        ...
        for index in index_sets:
            ...
            if index in self.energy_system.TIME_STEP_TYPES:
